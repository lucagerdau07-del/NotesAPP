...
export function titleFromFileName(name) {
  ...
  return (
    (dot > 0 ? trimmed.slice(0, dot) : trimmed).trim() ||
...
export function normalizeImportMime(file) {
  ...
  if (
    SUPPORTED_MIME.has(declared) &&
    fromExtension &&
    declared !== fromExtension
  ) {
    throw new ImportFailure(
      "type-mismatch",
      "Dateiendung und Dateityp widersprechen sich.",
    );
  }
...
export function validateSingleImport(files) {
  ...
  if (values.length !== 1)
    throw new ImportFailure(
      ...
      "Bitte jeweils nur eine Datei importieren.",
    );
  ...
  if (!mimeType)
    throw new ImportFailure(
      "unsupported-type",
      "Unterstützt werden PDF-, PNG- und JPEG-Dateien.",
    );
  if (selected.size === 0)
    throw new ImportFailure("empty-file", "Die Datei ist leer.");
  if (selected.size > MAX_IMPORT_BYTES)
    throw new ImportFailure(
      "file-too-large",
      "Die Datei ist größer als 100 MB.",
    );
  return {
    file: selected,
    mimeType,
...
export function toPageDescriptors(noteId, sourcePages) {
  if (!Array.isArray(sourcePages) || sourcePages.length === 0) {
    throw new ImportFailure(
      "no-pages",
      "Das Dokument enthält keine darstellbare Seite.",
    );
  }
  return sourcePages.map(({ width, height }, index) => {
    if (!(width > 0) || !(height > 0))
      throw new ImportFailure(
        "invalid-page-size",
        "Eine Seite hat ungültige Abmessungen.",
      );
    return {
