...
describe('Task 4 - WritingZone', () => {
  ...
  it('maps focus viewport points into page-local coordinates and commits through the controller', () => {
    ...
    render(<WritingZone inkController={inkController} focusBoxState={focusBoxState} toolbarState={toolbarState} />);
    const canvas = screen.getByTestId('focus-ink-canvas');
    canvas.getBoundingClientRect = () => ({ left: 10, top: 20, width: 800, height: 600 });

    fireEvent.pointerDown(canvas, {
      pointerId: 4, pointerType: 'pen', clientX: 210, clientY: 170,
    });
    fireEvent.pointerMove(canvas, {
      pointerId: 4, pointerType: 'pen', clientX: 610, clientY: 470,
    });
    ...
    expect(inkController.commitStroke.mock.calls[0][0]).toMatchObject({
      pageId: 'page-2',
      points: [{ x: 200, y: 125 }, { x: 400, y: 275 }],
    });
  });
  ...
  it('renders only the selected page with focus offsets and redraws on resize', () => {
    ...
    const inkController = controller([
      {
        id: 'hidden', pageId: 'page-1', tool: 'pen', color: '#fff', width: 3, opacity: 1,
        points: [{ x: 120, y: 70 }, { x: 140, y: 90 }],
      },
      {
        id: 'visible', pageId: 'page-2', tool: 'pen', color: '#fff', width: 3, opacity: 1,
        points: [{ x: 150, y: 75 }, { x: 200, y: 100 }],
      },
    ]);

    const view = render(<WritingZone
      inkController={inkController}
      focusBoxState={{ focusBox: { pageId: 'page-2', x: 100, y: 50, width: 400, height: 300 } }}
