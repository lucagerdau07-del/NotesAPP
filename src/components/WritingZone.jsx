...
export default function WritingZone({ inkController, focusBoxState, toolbarState }) {
  ...
  const mapPoint = useCallback(event => {
  ...
  redrawInkCanvasRef.current = () => {
    ...
    if (focusBox && focusBox.width > 0 && focusBox.height > 0) {
      ...
      const visibleStrokes = inkDocument.strokes.filter(stroke => stroke.pageId === focusBox.pageId);
      if (inkPointer.draftStroke
        && inkPointer.draftStroke.pageId === focusBox.pageId
        && inkTool !== 'stroke-eraser') {
        visibleStrokes.push(inkPointer.draftStroke);
      }
      visibleStrokes.forEach(stroke => renderInkStroke(context, stroke, {
  ...
  const getPadBackgroundStyles = () => {
    if (paperStyle === 'blank') {
      return { backgroundImage: 'none', backgroundSize: 'auto', backgroundPosition: '0px 0px' };
    }
