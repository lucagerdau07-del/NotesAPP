...
export function reducePointerInput(
  ...
  // Two un-blocked touches are a two-finger gesture, in every mode. Without a
  // digitizer the tip is a touch too, and a hand resting beside it reads the
  // same — only a palm-sized contact patch can tell those apart. Waiting for
  // the pair to move before deciding meant one finger of every pinch drew.
  ...
  if (
    ...
    // A second finger is a gesture, so the draft goes and gestures lock.
    return {
      state: {
        ...nextState,
        drawingPointerId: null,
