...
describe('contact classifier', () => {
  ...
  it('elects the smallest contact and condemns the palm-sized one', () => {
    const contacts = track([
  ...
  it('re-elects when a smaller contact lands later', () => {
    ...
    expect(classifyContacts(contacts, CONTACT_DEFAULTS, 1_010).electedId).toBe(2);
  });

  it('never condemns a fingertip-sized contact, however it lost the election', () => {
    // Two fingertips are two fingers: the policy treats the pair as a gesture.
    // Branding the one that rested (an anchored pinch finger) or moved less a
    // palm is what let the other one draw through the pinch.
    const contacts = track([
    ...
    expect(classifyContacts(contacts, CONTACT_DEFAULTS, 1_400)).toEqual({ electedId: 2, palmIds: [] });
  });
  ...
  it('condemns every palm-sized contact', () => {
    const contacts = track([
